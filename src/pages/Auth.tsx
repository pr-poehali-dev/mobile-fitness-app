...
const REGISTER_IMAGE = "https://cdn.poehali.dev/projects/c79ec5d9-7f1d-4fc3-a1ee-1f3cb89b439a/files/5f9e47e4-8760-4aff-ba66-2baf8ea0c8ba.jpg";
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="relative flex-1 overflow-hidden">
        <img src={HERO_IMAGE} alt="Фитнес" className="w-full h-full object-cover object-center animate-fade-in" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-black/80" />
        <div className="absolute top-14 left-6">
          <span className="font-display text-3xl text-white italic tracking-wide">Движение</span>
        </div>
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-10">
          <p className="opacity-0-start animate-fade-up delay-100 font-body text-white/60 text-xs tracking-widest uppercase mb-3">Твой личный фитнес</p>
          ...
          <div className="opacity-0-start animate-fade-up delay-400 flex flex-col gap-3">
            <button onClick={onRegister} className="w-full py-4 bg-white text-stone-800 rounded-2xl font-body font-semibold text-base transition-all active:scale-[0.98] shadow-lg">
              Создать аккаунт
            </button>
            <button onClick={onLogin} className="w-full py-4 bg-white/15 backdrop-blur-sm text-white border border-white/25 rounded-2xl font-body font-semibold text-base transition-all active:scale-[0.98]">
              Войти
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="relative h-52 overflow-hidden">
        ...
        <button onClick={onBack} className="absolute top-12 left-5 w-9 h-9 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
      ...
      <div className="flex-1 px-6 pt-6 pb-10">
        <p className="font-body text-sm text-muted-foreground mb-8">Войдите, чтобы продолжить тренировки</p>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="font-body text-xs text-muted-foreground uppercase tracking-wider mb-2 block">Email</label>
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2"><Icon name="Mail" size={16} className="text-muted-foreground" /></div>
          ...
          <div>
            ...
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2"><Icon name="Lock" size={16} className="text-muted-foreground" /></div>
          ...
          <button type="submit" className="w-full py-4 bg-primary text-white rounded-2xl font-body font-semibold text-base mt-2 transition-all active:scale-[0.98] shadow-md shadow-primary/20">
            Войти
          </button>
        </form>
        <div className="mt-6 text-center">
          <span className="font-body text-sm text-muted-foreground">Нет аккаунта? </span>
          <button onClick={onRegister} className="font-body text-sm text-primary font-semibold">Зарегистрироваться</button>
        </div>
      </div>
    </div>
  );
...
function RegisterScreen({ onBack, onAuth, onLogin }: { onBack: () => void; onAuth: AuthProps["onAuth"]; onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      {/* ШАГ 1 — фото девушки с гантелями + форма */}
      {step === 1 && (
        <div className="flex flex-col min-h-screen">
          {/* Фото — верхняя часть */}
          <div className="relative overflow-hidden flex-shrink-0" style={{ height: "45vh", minHeight: 220 }}>
            <img
              src={REGISTER_IMAGE}
            ...
            <div className="absolute inset-0" style={{
              background: "linear-gradient(to bottom, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.0) 50%, hsl(40,20%,97%) 100%)"
            }} />
            {/* Назад */}
            <button onClick={stepBack} className="absolute top-10 left-5 w-9 h-9 bg-white/25 backdrop-blur-sm rounded-full flex items-center justify-center">
              <Icon name="ArrowLeft" size={16} className="text-white" />
            </button>
            {/* Теги */}
            <div className="absolute bottom-8 left-5 flex gap-2">
              {[
                { icon: "Dumbbell", label: "Сила" },
                { icon: "Zap", label: "Энергия" },
                { icon: "Heart", label: "Здоровье" },
              ].map((t) => (
                <div key={t.label} className="flex items-center gap-1.5 bg-white/25 backdrop-blur-sm rounded-full px-3 py-1.5">
                  <Icon name={t.icon as any} size={11} className="text-white" />
                  <span className="font-body text-white text-xs font-medium">{t.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 px-6 pt-5 pb-8 overflow-y-auto">
            ...
            <h2 className="font-display text-4xl text-foreground font-light leading-tight mb-1 animate-fade-up">
              Создай<br /><em className="text-primary">аккаунт</em>
            </h2>
            <p className="font-body text-sm text-muted-foreground mb-5 animate-fade-up delay-100">Пара минут — и ты внутри</p>
            ...
            <div className="flex flex-col gap-4 animate-fade-up delay-200">
              ...
              <div>
                ...
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2"><Icon name="User" size={16} className="text-muted-foreground" /></div>
              ...
              <button onClick={handleStep1}
                className="w-full py-4 bg-primary text-primary-foreground rounded-2xl font-body font-semibold text-base transition-all active:scale-[0.98] shadow-md shadow-primary/20">
                Продолжить
              </button>

              <div className="text-center pb-2">
                <span className="font-body text-sm text-muted-foreground">Уже есть аккаунт? </span>
                <button onClick={onLogin} className="font-body text-sm text-primary font-semibold">Войти</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {step !== 1 && (
        <div className="flex flex-col min-h-screen">
          {/* Хедер */}
          <div className="relative pt-12 pb-3 px-6 flex-shrink-0">
            <button onClick={stepBack} className="w-9 h-9 bg-white border border-border rounded-full flex items-center justify-center shadow-sm">
          ...
          <div className="px-6 flex-shrink-0">
            ...
            <p className="font-body text-xs text-muted-foreground uppercase tracking-widest mb-1">Шаг {step} из 3</p>
          </div>
          ...
          <div className="flex-1 px-6 pb-10 overflow-y-auto">
            ...
            {step === 2 && (
              <div className="animate-fade-up">
                <h2 className="font-display text-4xl text-foreground font-light leading-tight mb-1 mt-3">
                ...
                <div className="flex flex-col gap-3">
                  {goals.map((g) => (
                    <button key={g.id} type="button" onClick={() => setGoal(g.id)}
                      className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all active:scale-[0.99] ${goal === g.id ? "border-primary bg-primary/5" : "border-border bg-white"}`}>
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${goal === g.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                        <Icon name={g.icon as any} size={18} />
                      </div>
                      <div className="text-left">
                        <p className={`font-body font-semibold text-sm ${goal === g.id ? "text-primary" : "text-foreground"}`}>{g.label}</p>
                        <p className="font-body text-xs text-muted-foreground">{g.desc}</p>
                      </div>
                      {goal === g.id && <div className="ml-auto"><Icon name="CheckCircle2" size={18} className="text-primary" /></div>}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-3">
                  {levels.map((l) => (
                    <button key={l.id} type="button" onClick={() => setLevel(l.id)}
                      className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all active:scale-[0.99] ${level === l.id ? "border-primary bg-primary/5" : "border-border bg-white"}`}>
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all ${level === l.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                        <Icon name={l.icon as any} size={18} />
                      </div>
                      <div className="text-left">
                        <p className={`font-body font-semibold text-sm ${level === l.id ? "text-primary" : "text-foreground"}`}>{l.label}</p>
                        <p className="font-body text-xs text-muted-foreground">{l.desc}</p>
                      </div>
                      {level === l.id && <div className="ml-auto"><Icon name="CheckCircle2" size={18} className="text-primary" /></div>}
                    </button>
                  ))}
                </div>
